...
class _DebtScreenState extends State<DebtScreen> {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (_, __) {
          ...
          final monthlyDebtPayment = store.debts.fold<double>(0, (sum, item) => sum + item.monthlyPayment);
          return ScreenShell(
            title: 'Debt',
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              SectionCard(title: 'Debt Strategy', child: Text('Total debt: ${money(store.totalDebt)}. Prioritize high-interest balances and protect monthly cash flow.', style: const TextStyle(color: AppColors.textMuted, height: 1.5))),
              const SizedBox(height: 14),
              SectionCard(title: 'Add debt', child: Column(children: [
                TextField(controller: title, decoration: const InputDecoration(labelText: 'Debt name', prefixIcon: Icon(Icons.credit_card_outlined))),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(child: TextField(controller: balance, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Balance'))),
                  const SizedBox(width: 10),
                  Expanded(child: TextField(controller: payment, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Monthly payment'))),
                ]),
                const SizedBox(height: 12),
                ...
                const SizedBox(height: 14),
                SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: addDebt, icon: const Icon(Icons.add), label: const Text('Add debt'))),
              ])),
              const SizedBox(height: 14),
